...
$match_query = mysqli_query($conn, "
  SELECT u.username, u.unique_id FROM matches m
...
<div class="container centered">
  ...
  <?php if ($matched_user) : ?>
    ...
    <p class="puppy-logo-text">Đang chat với: <?= $matched_user['username'] ?></p>
  <?php else: ?>
    <div class="text">Bạn chưa có ai ghép đôi.</div>
  <?php endif; ?>
  ...
  <div class="chat-box" id="chatBox">
    ...
    $message_query = mysqli_query($conn, "
      ...
      WHERE (incoming_msg_id = $current_id AND outgoing_msg_id = {$matched_user['unique_id']})
      OR (incoming_msg_id = {$matched_user['unique_id']} AND outgoing_msg_id = $current_id)
      ORDER BY msg_id ASC
    ");

    while ($message = mysqli_fetch_assoc($message_query)) {
      // Xác định xem đây là tin nhắn gửi đi hay nhận vào
      $message_class = ($message['outgoing_msg_id'] == $current_id) ? 'outgoing' : 'incoming';
      ?>
      <div class="chat <?= $message_class ?>">
        <div class="details">
          <?php if ($message['image']) : ?>
            <img class="chat-image" src="../uploads/<?= $message['image'] ?>" alt="Image Message" />
          <?php endif; ?>
  ...
  <div class="chat-input">
    <input type="text" id="message" placeholder="Chat anything..." <?= !$matched_user ? 'disabled' : '' ?> />
    <!-- Nút gửi ảnh -->
    ...
    <input type="file" id="imageInput" accept="image/*" style="display: none;" <?= !$matched_user ? 'disabled' : '' ?> />

    <button id="chatbotBtn">
      <img src="../img/chatbot_icon.png" alt="Chatbot" class="chatbot-icon" />
    </button>

    <button id="sendBtn" <?= !$matched_user ? 'disabled' : '' ?>>
